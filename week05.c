...
int main(){
    ...
    while(c--){
        ...
        isPromoted = (move[2] == '=') ? move[3] : (move[4] == '=') ? move[5] : '0';
        ...
        fprintf(outfile,"INSERT INTO Chess VALUES (%d, \"%s\", \"%s\", \"%s\", \"%s\", %d, %d ,%d, %d, %c);\n", mv, "White", piece, start, end, isCap, isCas, isCheck, isCMate, isPromoted);
        
        if(isCas){
            if(!strcmp(end,"g1")){
                fprintf(outfile,"INSERT INTO Chess VALUES (%d, \"%s\", \"%s\", \"%s\", \"%s\", %d, %d ,%d, %d, %c);\n", mv, "White", "Rook", "h1", "f1", isCap, isCas, isCheck, isCMate, isPromoted);
            }
            else{
                fprintf(outfile,"INSERT INTO Chess VALUES (%d, \"%s\", \"%s\", \"%s\", \"%s\", %d, %d ,%d, %d, %c);\n", mv, "White", "Rook", "a1", "d1", isCap, isCas, isCheck, isCMate, isPromoted);
            }
        }
        ...
        if(!strcmp(start,"b8") && !strcmp(end,"c8")){
            strcpy(start, "a8");
        }

        if(mv != 45){
            fprintf(outfile,"INSERT INTO Chess VALUES (%d, \"%s\", \"%s\", \"%s\", \"%s\", %d, %d ,%d, %d, %c);\n", mv, "Black", piece, start, end, isCap, isCas, isCheck, isCMate, isPromoted);
            if(isCas){
                if(!strcmp(end,"g1")){
                    fprintf(outfile,"INSERT INTO TChess VALUES (%d, \"%s\", \"%s\", \"%s\", \"%s\", %d, %d ,%d, %d, %c);\n", mv, "White", "Rook", "h1", "f1", isCap, isCas, isCheck, isCMate, isPromoted);
                }
                else{
                    fprintf(outfile,"INSERT INTO Chess VALUES (%d, \"%s\", \"%s\", \"%s\", \"%s\", %d, %d ,%d, %d, %c);\n", mv, "White", "Rook", "a1", "d1", isCap, isCas, isCheck, isCMate, isPromoted);
                }
